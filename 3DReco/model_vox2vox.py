...
class UNet3DWithAttenuation(nn.Module):
    def __init__(self, att_num_classes=4, out_channels=1, final_sigmoid=True):
        super(UNet3DWithAttenuation, self).__init__()

    
        # Define the combined branch (main UNet3D)
        self.UNet3D_att = UNet3D_att(in_channels=1 + att_num_classes, out_channels=out_channels, final_sigmoid=final_sigmoid)

    def forward(self, pet_img, att_map):
        # Get the embedding for each voxel in the attenuation map

        att_map = F.one_hot(att_map.long(), num_classes=4).permute(0, 5, 2, 3, 4, 1).float()
        att_map = att_map.squeeze(-1)

        # Concatenate PET image with the embedded attenuation map
        combined_input = torch.cat((pet_img, att_map), dim=1)
        output, _ = self.UNet3D_att(combined_input)
        return output
...
class Discriminator(nn.Module):
    ...
    def forward(self, img_A, img_B, att_map):
        att_map = F.one_hot(att_map.long(), num_classes=4).permute(0, 5, 2, 3, 4, 1).float()
        materials = att_map.squeeze(-1)
        
        # Concatenate image and condition image by channels to produce input
        img_input = torch.cat((img_A, img_B), 1)
        img_input = torch.cat((img_input, materials), 1)
...
if __name__ == '__main__':
    ...
    model = UNet3DWithAttenuation().to(device)
    rand_pet_img = torch.randn((1, 1, 64, 64, 64)).to(device)
    rand_att_map = torch.randint(0, 4, (1, 64, 64, 64)).to(device)
    output = model(rand_pet_img, rand_att_map)
    print(output.shape)
